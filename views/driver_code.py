...
class Mainwindow(QMainWindow):
    def __init__(self):
        ...
        self.orders_columns = ["Order_ID", "Order_Name", "Order_Date", "Order_Address", "Order_Item", "Order_Quantity", "Order_Total_Price"]
        ...
        self.remove_food_Btn.clicked.connect(lambda: remove_food(self))
        self.uploadPhoto_Btn.clicked.connect(lambda: upload_photo(self))
        if self.checking_the_connection(self.adminTable):
            self.adminTable.clicked.connect(lambda: self.get_whole_row(self.adminTable.currentIndex(), self.adminTable))
        self.admin_search_Btn.clicked.connect(lambda: search_food(self))
    ...
    def changing_adminoption_stack_PageNo(self, index):
        hide_wdgt = self.findChild(QWidget, "search_sort_option_wdgt")
        if index == 0:
            hide_wdgt.show()
        else:
            hide_wdgt.hide()
    ...
    # -------------------- Load admin Tables -------------------- #
    def view_foods(self, foods_list = Menu._food_list):
        self.adminTable.setModel(FoodTableModel(foods_list, self.foods_columns))
